...
public class AStarSousCheminTest {
...
public static void scenarioTest(String mapName, String pathName, String subpathName, Mode mode) throws Exception {
		...
		ShortestPathAlgorithm AStarSubAlgo = new AStarAlgorithm(subtestdata);

		//Create the solutions
		ShortestPathSolution AStarSolution = AStarAlgo.doRun();
		ShortestPathSolution AStarSubSolution = AStarSubAlgo.doRun();

		//Get the shortest paths
		Path dPath = AStarSolution.getPath();
		Path sdPath = AStarSubSolution.getPath();
...
//		int debut = pathArcs.indexOf(sdPath.getArcs().get(0));
		...
		if( debut == -1) {
			throw new Exception("The origin of the subpath is not in the path");
		} else {
			boolean same = true;
			float length = 0;
			double time = 0.0;
